...
@Component
public class AlterarPostagemValidator {
    public void validar(Usuario usuarioLogado, Postagem post) {
        if (!usuarioLogado.getId().equals(post.getUsuario().getId())) {
            throw new ResponseStatusException(BAD_REQUEST, "Não é possível alterar o post selecionado, pois ele não pertence ao usuário logado.");
        }
    }
}
